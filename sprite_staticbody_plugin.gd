...
func _on_selection_changed() -> void:
	...
	# Case 2: Selected node is a StaticBody2D
	if node is StaticBody2D:
		var found_sprite: Sprite2D = null
...
func _forward_canvas_draw_over_viewport(overlay: Control) -> void:
	if not preview_active or not is_instance_valid(preview_target_node):
		return
	if preview_polygons.is_empty():
		return
	Preview.draw_preview(overlay, preview_target_node, preview_polygons, preview_options)
...
func _calculate_preview() -> void:
	preview_polygons.clear()
	if not is_instance_valid(dock) or not is_instance_valid(preview_target_node):
		update_overlays()
		return
	...
	if preview_target_node is Sprite2D:
	...
	if tex:
		...
		var total_pts = 0
		for p in preview_polygons: total_pts += p.size()
...
func _convert_sprite_to_body(sprite: Sprite2D, options: Dictionary, scene_root: Node) -> void:
	...
	# Determine body type / script based on preset
	var preset = options.get("preset", 0)
	var body = StaticBody2D.new()
	body.name = "%sBody" % sprite.name
	
	if preset == 1: # Rotating Maze / Obstacle
	...
	# Execute via Undo/Redo
	var ur = get_undo_redo()
	ur.create_action("Convert Sprite to StaticBody")
	ur.add_do_method(self, "_do_convert_sprite", orig_parent, orig_index, body, sprite, shadow_sprite, col_nodes, scene_root)
	ur.add_undo_method(self, "_undo_convert_sprite", orig_parent, orig_index, body, sprite, shadow_sprite, col_nodes, orig_xform)
	ur.commit_action()
	
	var shadow_msg = " + Real Shadow" if is_instance_valid(shadow_sprite) else ""
	dock.set_status_message("StaticBody2D '%s' created with %d collision polygon(s)%s!" % [body.name, col_nodes.size(), shadow_msg])
	EditorInterface.get_selection().clear()
	EditorInterface.get_selection().add_node(body)

func _do_convert_sprite(orig_parent: Node, orig_index: int, body: StaticBody2D, sprite: Sprite2D, shadow_sprite: Sprite2D, col_nodes: Array[CollisionPolygon2D], scene_root: Node) -> void:
	# Add body to original parent at same sibling index
	...
	# Reparent main sprite under body
...
func _undo_convert_sprite(orig_parent: Node, orig_index: int, body: StaticBody2D, sprite: Sprite2D, shadow_sprite: Sprite2D, col_nodes: Array[CollisionPolygon2D], orig_xform: Transform2D) -> void:
	...
	# Move sprite back to original parent
...
func _create_body_from_texture(texture: Texture2D, path: String, options: Dictionary, scene_root: Node) -> void:
	...
	ur.add_do_method(self, "_do_add_body_tree", scene_root, body, sprite, shadow_sprite, col_nodes)
	ur.add_undo_method(self, "_undo_add_body_tree", scene_root, body)
	ur.commit_action()
	
	var shadow_msg = " + Real Shadow" if is_instance_valid(shadow_sprite) else ""
	dock.set_status_message("Created StaticBody '%s' with %d collision polygon(s)%s!" % [body.name, col_nodes.size(), shadow_msg])
	EditorInterface.get_selection().clear()
	EditorInterface.get_selection().add_node(body)

func _do_add_body_tree(parent: Node, body: StaticBody2D, sprite: Sprite2D, shadow_sprite: Sprite2D, col_nodes: Array[CollisionPolygon2D]) -> void:
...
func _undo_add_body_tree(parent: Node, body: StaticBody2D) -> void:
...
func _on_regenerate_collision(options: Dictionary) -> void:
	...
	if not is_instance_valid(body):
		if is_instance_valid(sprite) and sprite.get_parent() is StaticBody2D:
			body = sprite.get_parent()
		else:
			dock.set_status_message("No StaticBody2D selected to regenerate collision.", true)
			return
	...
	var scene_root = EditorInterface.get_edited_scene_root()
	var ur = get_undo_redo()
	ur.create_action("Regenerate Collision & Shadow on StaticBody")
...
func _do_replace_collisions(body: StaticBody2D, old_nodes: Array[CollisionPolygon2D], new_nodes: Array[CollisionPolygon2D], old_shadow: Sprite2D, new_shadow: Sprite2D, root: Node) -> void:
...
func _undo_replace_collisions(body: StaticBody2D, old_nodes: Array[CollisionPolygon2D], new_nodes: Array[CollisionPolygon2D], old_shadow: Sprite2D, new_shadow: Sprite2D, root: Node) -> void:
	for new_node in new_nodes:
...
func _on_remove_collision() -> void:
	var body = dock.target_body
	if not is_instance_valid(body) and is_instance_valid(dock.target_sprite) and dock.target_sprite.get_parent() is StaticBody2D:
		body = dock.target_sprite.get_parent()
		
	if not is_instance_valid(body):
		dock.set_status_message("No StaticBody2D selected.", true)
		return
	...
	var scene_root = EditorInterface.get_edited_scene_root()
	var ur = get_undo_redo()
	ur.create_action("Remove Collision from StaticBody")
...
func _do_remove_collisions(body: StaticBody2D, col_nodes: Array[CollisionPolygon2D]) -> void:
	for col in col_nodes:
		if is_instance_valid(col) and col.get_parent() == body:
			body.remove_child(col)

func _undo_remove_collisions(body: StaticBody2D, col_nodes: Array[CollisionPolygon2D], root: Node) -> void:
	for col in col_nodes:
